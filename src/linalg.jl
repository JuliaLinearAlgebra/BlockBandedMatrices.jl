...
#
#
# strides(V::SubBlockBandedMatrix{<:Any,BlockRange1,Block1}) =
#     (1,parent(V).block_sizes.block_strides[Int(parentindices(V)[2].block)])
#
# function unsafe_convert(::Type{Ptr{T}}, V::SubBlockBandedMatrix{T,BlockRange1,Block1}) where T
#     A = parent(V)
...
# strides(V::SubBlockBandedMatrix{T,BlockIndexRange1,BlockIndexRange1}) =
#     (1,parent(V).block_sizes.block_strides[Int(parentindices(V)[2].block.block)])

# MemoryLayout(V::SubBlockBandedMatrix{T,BlockIndexRange1,BlockIndexRange1}) = ColumnMajor()
