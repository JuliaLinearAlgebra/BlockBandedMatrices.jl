...
function LinearAlgebra.mul!(c::AbstractBlockVector{T},
    ...
    @sync for (p, proc) in enumerate(procs(A))
        ...
        @async begin
          remotecall_wait(proc, start:stop) begin irange
            @inbounds for i in irange
                fill!(c.blocks[i], zero(eltype(block)))
                for j = max(1, i - A.l):min(nblocs(A, 2), i + A.u)
                    BLAS.gbmv!('N', size(view(A, Block(i, j)), 1), A.λ, A.μ, one(T),
...
function testme()
  ...
  @testset "shared array backend" begin
    ...
    @testset "Multiplication" begin
       ...
       Ablock = adapt(SharedArray, A)
       cblock = adapt(SharedArray,
                      PseudoBlockArray(Array{Float64, 1}(undef, size(A, 1)), n))
       cblock .= rand.()
       xblock = adapt(SharedArray, x)
